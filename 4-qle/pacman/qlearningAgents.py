...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        #test if random action should be taken (proba epsilon):
        go_random = util.flipCoin(self.epsilon)
        if(go_random):
    ...
    def update(self, state, action, nextState, reward):
        ...
        # q_old = self.qtable[(state, action)]
        # self.qtable[(state, action)] = self.alpha * (reward + self.discount * self.getValue(nextState)) + (1-self.alpha) * q_old        
        # util.raiseNotDefined()
